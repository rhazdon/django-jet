...
class Dashboard(object):
    ...
    def load_modules(self):
        module_models = UserDashboardModule.objects.filter(
            app_label=self.app_label, user=self.context["request"].user.pk
        ).all()

        if len(module_models) == 0:
            module_models = self.create_initial_module_models(
...
class DefaultIndexDashboard(Dashboard):
    ...
    def init_with_context(self, context):
        ...
        self.children.append(
            modules.AppList(_("Applications"), exclude=("auth.*",), column=1, order=0)
        )

        # append an app list module for "Administration"
        self.children.append(
            modules.AppList(_("Administration"), models=("auth.*",), column=2, order=0)
        )
...
class DefaultAppIndexDashboard(AppIndexDashboard):
    def init_with_context(self, context):
        ...
        self.children.append(
            modules.ModelList(
                title=_("Application models"), models=self.models(), column=0, order=0
            )
        )
        self.children.append(
            modules.RecentActions(
                include_list=self.get_app_content_types(), column=1, order=0
            )
        )
