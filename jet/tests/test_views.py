...
class ViewsTestCase(TestCase):
    ...
    def test_module_update_view(self):
        ...
        response = self.admin.get(
            reverse("jet-dashboard:update_module", kwargs={"pk": module.pk})
        )
        ...
        self.admin.post(
            reverse("jet-dashboard:update_module", kwargs={"pk": module.pk}), post
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_add_bookmark_view(self):
        ...
        response = self.admin.post(
            reverse("jet:add_bookmark"), {"url": url, "title": title}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_remove_bookmark_view(self):
        ...
        bookmark = Bookmark.objects.create(
            url=url, title=title, user=self.admin_user.pk
        )
    ...
    def test_toggle_application_pin_view(self):
        ...
        response = self.admin.post(
            reverse("jet:toggle_application_pin"), {"app_label": app_label}
        )
        self.assertEqual(response.status_code, 200)
        response = json.loads(response.content.decode())
        self.assertFalse(response["error"])
    ...
    def test_update_dashboard_modules_view(self):
        ...
        response = self.admin.post(
            reverse("jet-dashboard:update_dashboard_modules"),
            {
                "app_label": "",
                "modules": json.dumps(
                    [
    ...
    def test_add_user_dashboard_module_view(self):
        response = self.admin.post(
            reverse("jet-dashboard:add_user_dashboard_module"),
    ...
    def test_add_user_app_dashboard_module_view(self):
        ...
        response = self.admin.post(
            reverse("jet-dashboard:add_user_dashboard_module"),
            {"app_label": app_label, "type": "available_children", "module": 0},
        )
    ...
    def test_update_dashboard_module_collapse_view(self):
        ...
        response = self.admin.post(
            reverse("jet-dashboard:update_dashboard_module_collapse"),
    ...
    def test_remove_dashboard_module_view(self):
        ...
        response = self.admin.post(
            reverse("jet-dashboard:remove_dashboard_module"), {"id": module.pk}
        )
        self.assertEqual(response.status_code, 200)
        response = json.loads(response.content.decode())
        self.assertFalse(response["error"])
